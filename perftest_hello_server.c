...
void run_server(struct rdma_resource *res) {
    ...
    // Wait for client to send a message (for demonstration, we'll use the first BUFFER_SIZE bytes)
    recv(client_fd, (void *)res->cuda_buffer, BUFFER_SIZE, 0);
    printf("Received message from client (GPU memory)\n");
...
int main(int argc, char **argv) {
    ...
    create_rdma_resources(&res, "mlx5_1");
    run_server(&res);
    return 0;
}